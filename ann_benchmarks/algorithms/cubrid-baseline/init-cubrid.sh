...
# Hand off to the base image's entrypoint. We override ENTRYPOINT above
# (so docker CMD would otherwise be run standalone), but ann-benchmarks
# passes CMD as bare args (e.g. "--dataset ... --algorithm ...") that are
# meant to be appended to the base ENTRYPOINT: python -u run_algorithm.py.
cd /home/app
